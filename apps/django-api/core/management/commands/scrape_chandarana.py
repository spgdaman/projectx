"""
python manage.py scrape_chandarana [--force] [--limit N] [--no-categorise]

Scrape Chandarana (Magento 2 API with Playwright fallback) and optionally
run category mapping. Prints real-time progress. Exits with code 1 on failure.
"""
import sys
import time
...
class Command(BaseCommand):
    help = 'Scrape Chandarana deals and write to staging -> products/deals.'
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write('[Chandarana] Starting scrape — strategy: API (Playwright fallback if needed)')
        ...
        try:
            ...
            self.stderr.write(self.style.ERROR(f'[Chandarana] Scrape failed: {exc}'))
            import traceback
            traceback.print_exc()
            sys.exit(1)
        ...
        if run is None:
            self.stdout.write(self.style.WARNING(
                '[Chandarana] Skipped — successful run already completed within last 2h. '
        ...
        self.stdout.write('')
        self.stdout.write('[Chandarana] Scrape complete:')
        ...
        if run.status == 'failed':
            self.stderr.write(self.style.ERROR('[Chandarana] Run finished with status=failed'))
            sys.exit(1)

        if no_categorise:
            self.stdout.write('[Chandarana] Skipping category mapping (--no-categorise).')
            return

        self.stdout.write('')
        self.stdout.write('[Chandarana] Running category mapping...')
        try:
            from django.core.management import call_command
            call_command('map_categories', retailer='Chandarana', stdout=self.stdout)
        except Exception as exc:
            self.stderr.write(self.style.WARNING(f'[Chandarana] Category mapping failed: {exc}'))

        self.stdout.write(self.style.SUCCESS('[Chandarana] Done.'))
