"""
python manage.py scrape_naivas [--force] [--limit N] [--no-categorise]

Scrape Naivas (Playwright-only) and optionally run category mapping.
...
class Command(BaseCommand):
    help = 'Scrape Naivas deals and write to staging -> products/deals.'
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write('[Naivas] Starting scrape — strategy: Playwright (API not available)')
        ...
        try:
            ...
            if limit:
                # Monkey-patch write_to_staging to stop after limit rows
            ...
            self.stderr.write(self.style.ERROR(f'[Naivas] Scrape failed: {exc}'))
            import traceback
            traceback.print_exc()
            sys.exit(1)
        ...
        if run is None:
            self.stdout.write(self.style.WARNING(
                '[Naivas] Skipped — successful run already completed within last 2h. '
        ...
        self.stdout.write('')
        self.stdout.write('[Naivas] Scrape complete:')
        ...
        if run.status == 'failed':
            self.stderr.write(self.style.ERROR('[Naivas] Run finished with status=failed'))
            sys.exit(1)

        if no_categorise:
            self.stdout.write('[Naivas] Skipping category mapping (--no-categorise).')
            return

        self.stdout.write('')
        self.stdout.write('[Naivas] Running category mapping...')
        try:
            from django.core.management import call_command
            call_command('map_categories', retailer='Naivas', stdout=self.stdout)
        except Exception as exc:
            self.stderr.write(self.style.WARNING(f'[Naivas] Category mapping failed: {exc}'))

        self.stdout.write(self.style.SUCCESS('[Naivas] Done.'))
