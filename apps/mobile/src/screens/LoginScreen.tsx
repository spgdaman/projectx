...
export default function LoginScreen({ navigation }: Props) {
  ...
  async function handleLogin() {
    if (!localPhone.trim() || !password) { setError('Enter phone and password.'); return; }
    setError('');
    setLoading(true);
      ...
      <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
        <ScrollView contentContainerStyle={s.container} keyboardShouldPersistTaps="handled">
          ...
          <View style={s.form}>
            ...
            <TouchableOpacity
              style={[s.btnPrimary, (loading || !localPhone || !password) && s.btnDisabled]}
              onPress={handleLogin}
              disabled={loading || !localPhone.trim() || !password}
              activeOpacity={0.8}
            >
...
const s = StyleSheet.create({
  ...
  phoneInput: {
    ...
    paddingHorizontal: 16, paddingVertical: 13, fontSize: 15, color: colors.textPrimary,
  },
  inputWrapper: {
    flexDirection: 'row', alignItems: 'center',
    backgroundColor: colors.surface, borderWidth: 1, borderColor: colors.border, borderRadius: 12,
  },
  inputWithToggle: { flex: 1, paddingHorizontal: 16, paddingVertical: 13, fontSize: 15, color: colors.textPrimary },
